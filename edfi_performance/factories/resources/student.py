...
from edfi_performance.api.client.program import ProgramClient
...
from ..utils import UniqueIdAttribute, formatted_date, current_year, RandomDateAttribute
...
class StudentFactory(APIFactory):
    ...
    electronicMails = factory.List([
        dict(
            ...
            electronicMailTypeDescriptor=build_descriptor('ElectronicMailType', 'Other'),
        ),
    ])
    identificationCodes = factory.List([
        dict(
            ...
            studentIdentificationSystemDescriptor=build_descriptor('StudentIdentificationSystem', 'State'),
        ),
    ])
    ...
    studentRaces = factory.List([
        dict(
            raceDescriptor=build_descriptor('Race', 'Black - African American')
        ),
    ])
    telephones = factory.List([
        dict(
            telephoneNumber='111-222-3333',
            telephoneNumberTypeDescriptor=build_descriptor('TelephoneNumberType', 'Home'),
        ),
    ])
...
class StudentCohortAssociationFactory(APIFactory):
    ...
    cohortReference = factory.Dict(dict(
...
class StudentDisciplineIncidentAssociationFactory(APIFactory):
    ...
    studentParticipationCodeDescriptor = build_descriptor('StudentParticipationCode', 'Perpetrator')


class StudentEducationOrganizationAssociationFactory(APIFactory):
    studentReference = factory.Dict(dict(studentUniqueId=111111))  # Default value for scenarios, but not in DB
    educationOrganizationReference = factory.Dict(
        dict(educationOrganizationId=LocalEducationAgencyClient.shared_education_organization_id()))  # Default ed org
    limitedEnglishProficiencyDescriptor = build_descriptor('LimitedEnglishProficiency', 'NotLimited')
    studentCharacteristics = factory.List([
        dict(
            studentCharacteristicDescriptor=build_descriptor('StudentCharacteristic', 'Immigrant'),
        ),
    ])
...
class StudentProgramAssociationFactory(APIFactory):
    studentReference = factory.Dict(dict(studentUniqueId=111111))  # Default value for scenarios, but not in DB
    educationOrganizationReference = factory.Dict(
        dict(educationOrganizationId=LocalEducationAgencyClient.shared_education_organization_id()))  # Prepopulated ed organization
    programReference = factory.Dict(dict(
        programTypeDescriptor=build_descriptor('ProgramType', 'Gifted and Talented'),
...
class StudentSchoolAssociationFactory(APIFactory):
    ...
    residencyStatusDescriptor = build_descriptor('ResidencyStatus', 'Resident of admin unit and school area')
...
class StudentTitleIPartAProgramAssociationFactory(APIFactory):
    ...
    beginDate = formatted_date(8, 23)
    titleIPartAParticipantDescriptor = build_descriptor('TitleIPartAParticipant', 'Public Targeted Assistance Program')
...
class StudentSpecialEducationProgramAssociationFactory(APIFactory):
    ...
    beginDate = formatted_date(8, 23)
    specialEducationSettingDescriptor = build_descriptor('SpecialEducationSetting',
...
class StudentAcademicRecordFactory(APIFactory):
    educationOrganizationReference = factory.Dict(
        dict(educationOrganizationId=SchoolClient.shared_elementary_school_id()))  # Prepopulated school
...
class StudentCompetencyObjectiveFactory(APIFactory):
    ...
    objectiveCompetencyObjectiveReference = factory.Dict(dict(
...
class StudentCTEProgramAssociationFactory(APIFactory):
    educationOrganizationReference = factory.Dict(
        dict(educationOrganizationId=LocalEducationAgencyClient.shared_education_organization_id()))  # Prepopulated ed org
    studentReference = factory.Dict(dict(studentUniqueId=StudentClient.shared_student_id()))  # Prepopulated student
    programReference = factory.Dict(dict(
...
class StudentGradebookEntryFactory(APIFactory):
    ...
    gradebookEntryReference = factory.Dict(dict(
        localCourseCode="ELA-01",
        schoolId=SchoolClient.shared_elementary_school_id(),
        schoolYear=current_year(),
        sectionIdentifier="ELA012017RM555",
    ...
    studentSectionAssociationReference = factory.Dict(dict(
        localCourseCode="ELA-01",
        schoolId=SchoolClient.shared_elementary_school_id(),
        schoolYear=current_year(),
        sectionIdentifier="ELA012017RM555",
...
class StudentInterventionAssociationFactory(APIFactory):
    studentReference = factory.Dict(dict(studentUniqueId=StudentClient.shared_student_id()))  # Prepopulated student
    interventionReference = factory.Dict(dict(
        educationOrganizationId=LocalEducationAgencyClient.shared_education_organization_id(),
...
class StudentInterventionAttendanceEventFactory(APIFactory):
    ...
    attendanceEventCategoryDescriptor = build_descriptor('AttendanceEventCategory', 'In Attendance')
...
class StudentProgramAttendanceEventFactory(APIFactory):
    ...
    studentReference = factory.Dict(dict(studentUniqueId=StudentClient.shared_student_id()))  # Prepopulated student
    attendanceEventCategoryDescriptor = build_descriptor('AttendanceEventCategory', 'Excused Absence')
