...
class OpenStaffPositionFactory(APIFactory):
    ...
    educationOrganizationReference = factory.Dict(dict(educationOrganizationId=SchoolClient.shared_elementary_school_id()))
    employmentStatusDescriptor = build_descriptor('EmploymentStatus', 'Substitute/temporary')
    staffClassificationDescriptor = build_descriptor('StaffClassification', 'Substitute Teacher')
...
class StaffFactory(APIFactory):
    ...
    highestCompletedLevelOfEducationDescriptor = build_descriptor('LevelOfEducationDescriptor', 'Master\'s')
...
class StaffAbsenceEventFactory(APIFactory):
    absenceEventCategoryDescriptor = build_descriptor('AbsenceEventCategory', 'Vacation')
...
class StaffLeaveFactory(APIFactory):
    staffReference = factory.Dict(dict(staffUniqueId=StaffClient.shared_staff_id()))  # Prepopulated staff record
    staffLeaveEventCategoryDescriptor = build_descriptor('StaffLeaveEventCategory', 'Bereavement')
    beginDate = RandomDateAttribute()
...
class StaffSectionAssociationFactory(APIFactory):
    ...
    staffReference = factory.Dict(dict(staffUniqueId=StaffClient.shared_staff_id()))  # Prepopulated staff record
    classroomPositionDescriptor = build_descriptor('ClassroomPosition', 'Teacher of Record')
