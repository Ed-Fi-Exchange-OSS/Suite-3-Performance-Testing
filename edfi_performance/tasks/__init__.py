...
class EdFiTaskSet(TaskSet):
    ...
    def __init__(self, parent, *args, **kwargs):
        if type(self) is EdFiTaskSet:
            raise NotImplementedError("Abstract class EdFiTaskSet cannot be instantiated")

        token = kwargs.pop('token', None)
        if token is None:
            token = getattr(parent, 'shared_token', None)

        super(EdFiTaskSet, self).__init__(parent, *args, **kwargs)

        self.generate_client_class()

        self._api_client = self.client_class(self.locust.host, token=token)

    def __getattr__(self, item):
        # Pass some attributes through to the EdFiAPIClient
        if (
                item in [                                     # CRUD calls
                    'login', 'get_item', 'get_list',
                    'create', 'update', 'delete', 'get_composite_list'
                ] or
                item == 'factory' or                          # Factory instance
                item.endswith('_client')                      # Dependency client
        ):
    ...
    def create_with_dependencies(self, client_class=None, **kwargs):
        ...
        client_instance = client_class(self.locust.host, token=self._api_client.token)
