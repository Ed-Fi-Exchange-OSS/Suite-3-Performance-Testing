...
from edfi_performance.api.client.graduation_plan import GraduationPlanClient
...
class StudentParentAssociationVolumeTest(EdFiVolumeTestBase):
    ...
    def run_association_scenarios(self):
        ...
        self.run_scenario('emergencyContactStatus', False, relationDescriptor=build_descriptor('Relation', 'Mother'))
...
class StudentSchoolAssociationVolumeTest(EdFiVolumeTestBase):
    ...
    def run_association_scenarios(self):
        ...
        graduation_plan_reference = {  # Prepopulated graduation plan
            ...
            'graduationSchoolYear':
                StudentSchoolAssociationVolumeTest.get_graduation_plan_school_year(self, school_id)
        }
        self.run_scenario('entryDate', RandomDateAttribute(),
                          exitWithdrawDate=formatted_date(9, 1),
                          exitWithdrawTypeDescriptor=build_descriptor('ExitWithdrawType', 'Transferred'))
        self.run_scenario('graduationPlanReference', graduation_plan_reference,
    ...
    def get_graduation_plan_school_year(cls, self, school_id):
        ...
        client_instance = GraduationPlanClient(self.locust.host, token=self._api_client.token)
        plan_type = urllib.quote_plus(build_descriptor('GraduationPlanType', 'Recommended'))
        query = "?graduationPlanTypeDescriptor={}&educationOrganizationId={}".format(plan_type, school_id)
        all_graduation_years = client_instance.get_list(query)
        if all_graduation_years:
            cls._graduation_year = all_graduation_years[0]['graduationSchoolYearTypeReference']['schoolYear']
        return cls._graduation_year
...
class StudentVolumeTest(EdFiVolumeTestBase):
    ...
    def run_student_scenarios(self):
        ...
        self.run_scenario('cityStreetName', '123 Cedar Street',
                          birthDate='1/1/2001',
                          firstName="Madison",
                          middleName="Mary",
                          lastSurname="Johnson",
    ...
    def _update_attribute(self, resource_id, resource_attrs, update_attribute_name, update_attribute_value, **kwargs):
        if update_attribute_name == 'cityStreetName':
            ...
            resource_attrs['telephones'][0][update_attribute_name] = update_attribute_value
...
class StudentTitleIPartAProgramAssociationVolumeTest(EdFiVolumeTestBase):
    @task
    def run_association_scenarios(self):
        self.run_scenario('titleIPartAParticipantDescriptor',
                          build_descriptor('TitleIPartAParticipant', 'Public Schoolwide Program'))
        self.run_scenario('titleIPartAParticipantDescriptor',
                          build_descriptor('TitleIPartAParticipant', 'Was not served'),
                          titleIPartAParticipantDescriptor=build_descriptor('TitleIPartAParticipant',
...
class StudentSpecialEducationProgramAssociationVolumeTest(EdFiVolumeTestBase):
    ...
    def run_association_scenarios(self):
        ...
        self.run_scenario('ideaEligibility', False,
                          ...
                          build_descriptor('SpecialEducationSetting', 'Inside regular class less than 40% of the day'))
...
class StudentProgramAssociationVolumeTest(EdFiVolumeTestBase):
    ...
    def run_association_scenarios(self):
        ...
        self.run_scenario('endDate', formatted_date(10, 20),
                          programReference__programTypeDescriptor=build_descriptor('ProgramType', program_type),
...
class StudentSchoolAttendanceEventVolumeTest(EdFiVolumeTestBase):
    ...
    def run_event_scenarios(self):
        ...
        self.run_scenario('attendanceEventReason', "No Note",
                          schoolId=SchoolClient.shared_high_school_id(),
                          eventDate=formatted_date(10, 5),
                          attendanceEventCategoryDescriptor=build_descriptor('AttendanceEventCategory',
                                                                             'Unexcused Absence'))
